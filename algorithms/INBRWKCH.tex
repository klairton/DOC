\begin{algorithm}[!tbh]
  \caption{Um algoritmo de aproximação para o problema \SbPRT{} em instâncias clássicas sem sinais e para $k \in [0..1]$.\label{algorithm:INBRWKCH}}
  \Entrada{Uma instância clássica sem sinais $\mathcal{I} = (\pi,\iota)$ e um valor de $k \in [0..1]$}
  \Saida{Uma sequência de reversões e transposições $S$, tal que $\pi \cdot S = \iota$ e $\frac{|S_{\rho}|}{|S|} \ge k$}
    Seja $S \gets ()$ \\
    \Enqto{$\pi \ne \iota$}{
      \Se {$\frac{|S_{\rho}|}{|S| + 1} \ge k$ e existe uma transposição $\tau$ tal que $\Delta b_1(\mathcal{I}, (\tau)) \le -1$}{
        $\pi \gets \pi \cdot \tau$ \\
        $S \gets S + (\tau)$ \\
      }\Senao{
        Seja $S'$ uma sequência de reversões (de tamanho um ou dois) que remove, na média, um breakpoint tipo um por operação~\cite{1995-kececioglu-sankoff} \\
        $\pi \gets \pi \cdot S'$ \\
        $S \gets S + S'$ \\
      }
    }
  \Retorna{S}
\end{algorithm}